...
Componentes ficam agrupados por domínio (`professional/`, `home/`, `layout/`), com `ui/` para blocos genéricos e reutilizáveis (`UiButton`, `UiIcon`, `UiSelect`, `UiChip`, `UiStatePanel`); o acesso à API passa por composables finos (`useProfessionals`, `useProfessionalFilters`, `useProfessionalDetail`) em vez de uma camada de serviço — com só dois endpoints, um composable por recurso já é a abstração inteira necessária.

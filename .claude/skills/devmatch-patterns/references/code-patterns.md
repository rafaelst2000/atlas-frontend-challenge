...
**Never name a handler file `index.get.ts` (or any `index.*`) when a dynamic sibling route shares its parent folder.** `server/api/professionals/index.get.ts` sitting next to `server/api/professionals/[id].get.ts` bundled to a wrongly-named chunk (`api/index.get.mjs` — missing the `professionals` segment entirely, collapsed as if it were `server/api/index.get.ts`) under `NITRO_PRESET=vercel`. Nitro's own h3 router still resolved the route correctly by object reference, so this looked cosmetic — but it was the actual cause of the bare `/api/professionals` 404 in production: the naming collision confused how Vercel's real build assembles that route's function. The fix was structural, not a routeRules tweak: renamed the file to `server/api/professionals.get.ts` (a plain sibling file, not an `index` inside the same folder as the dynamic route), which produces an unambiguous `api/professionals.get.mjs` chunk. If a list/detail pair like this is needed again, give the list handler its own file outside the dynamic segment's folder from the start.

**Verify any `routeRules` or route-file-structure change against the deployed preset, before *and* after the fix** — not just before shipping it: `rm -rf .output .nuxt .vercel && NITRO_PRESET=vercel npm run build`, then inspect `.vercel/output/config.json`'s `routes` array **and** the chunk file names under `.vercel/output/functions/**/chunks/routes/` for the path in question — a wrongly-named chunk is itself a signal, even if the route still resolves in `nitro.mjs`. Plain `npm run build` uses Nitro's `node-server` preset, which never generates this layer at all; it will build and serve every route correctly locally while the real Vercel deployment 404s or silently caches the wrong response. All three bugs here were only caught by testing the live `https://…vercel.app` URL directly, and the first fix *introduced* the second — re-check after fixing, not just before. `.vercel/` is git-ignored and disposable — regenerate it whenever this needs re-checking.

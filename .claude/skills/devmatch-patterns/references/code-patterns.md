...
**Verify any `routeRules` change against the deployed preset, before *and* after the fix** — not just before shipping it: `rm -rf .output .nuxt .vercel && NITRO_PRESET=vercel npm run build`, then inspect `.vercel/output/config.json`'s `routes` array for the path in question. Plain `npm run build` uses Nitro's `node-server` preset, which never generates this routing/ISR layer at all — it will build and serve every route correctly locally while the real Vercel deployment 404s or, worse, silently caches the wrong response. Both bugs above were only caught by testing the live `https://…vercel.app` URL directly; the fix for the first one *introduced* the second, so re-check after fixing, not just before. `.vercel/` is git-ignored and disposable — regenerate it whenever this needs re-checking.

...
**Reuse before repeating.** Don't write an inline `<svg>` — add the icon to `app/utils/icons.ts` and render it with `<UiIcon name="..." class="text-accent">` (color comes from `currentColor`, so it's a text-color class, not a `stroke` attribute). Don't hand-roll `class="btn btn-primary ... !py-3.25"` — use `<UiButton variant size to>` (`size="lg"` is the tall CTA; layout-only classes like `w-full`/`mt-6` still go on the call site). Error/empty panels are `<UiStatePanel icon title description tone>`, selects share `FilterSelect`/`SortSelect`, and the green "Disponível" pill is `ProfessionalAvailabilityBadge`. If a third copy of some markup appears, extract it the same way (component + test in `test/components/`, since every component carries the 90% coverage floor).

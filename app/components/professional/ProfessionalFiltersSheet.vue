...
<template>
  <div
    class="fixed inset-0 z-[500] flex items-end bg-[rgba(6,5,4,0.72)]"
    role="dialog"
    aria-modal="true"
    aria-label="Filtros"
  ...
  >
    ...
    <div class="relative max-h-[88vh] w-full animate-sheet-up overflow-y-auto rounded-t-panel bg-footer p-5 pb-[calc(20px+env(safe-area-inset-bottom))] shadow-[inset_0_1px_0_rgba(255,248,230,0.08),0_-12px_40px_rgba(0,0,0,0.6)]">
      <div class="mb-5 flex items-center">
        ...
        <button
          type="button"
          class="ml-auto flex size-8.5 items-center justify-center rounded-button border border-medium bg-card"
          aria-label="Fechar"
          @click="emit('close')"
        >
          <UiIcon
      ...
      <p class="mb-2.5 text-[13px] font-semibold text-primary">
        Especialidade
      </p>
      ...
      <div class="mt-5 flex gap-2.5">
        <button
          type="button"
          class="flex-1 rounded-button border border-subtle p-3 text-sm font-semibold text-body"
          @click="clear()"
        >
          Limpar filtros
        </button>

  ...
  <article class="card flex flex-col gap-3.5 p-5">
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="mt-0.75 text-[13px] text-body">{{ professional.role }}</p>
      </div>
      <span class="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-success/30 bg-success/10 px-2.5 py-1.25 text-label text-[#6dc48b]">
        <span class="size-1.25 rounded-full bg-success" aria-hidden="true" />Disponível
      </span>
    </div>
    ...
    <ul class="flex flex-wrap gap-1.5" aria-label="Tecnologias">
      <li v-for="tech in professional.techs" :key="tech" class="rounded-full border border-subtle px-2.5 py-1.25 font-mono text-label text-body">
        {{ tech }}
      </li>
    </ul>

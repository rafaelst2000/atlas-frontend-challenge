  ...
  <div class="flex min-w-0 flex-[3_1_420px] flex-col gap-5">
    <section aria-labelledby="sobre">
      ...
      <div class="flex flex-col gap-3.5 rounded-card bg-card p-6 shadow-card">
        <p v-for="(paragraph, index) in professional.about" :key="index" class="text-[15px] leading-[1.7] text-body [text-wrap:pretty]">{{ paragraph }}</p>
      </div>
    </section>
    ...
    <section aria-labelledby="tecnologias">
      ...
      <ul class="flex flex-wrap gap-2">
        <li v-for="tech in professional.techs" :key="tech" class="rounded-full bg-card px-4 py-2.25 font-mono text-[12.5px] text-primary shadow-[inset_0_1px_0_rgba(255,248,230,0.08),0_2px_12px_rgba(0,0,0,0.35)]">{{ tech }}</li>
      </ul>
    </section>
    ...
    <section aria-labelledby="servicos">
      ...
      <ul class="grid gap-3.5 [grid-template-columns:repeat(auto-fit,minmax(min(100%,220px),1fr))]">
        <li v-for="service in professional.services" :key="service.title" class="card flex flex-col gap-2.5 p-5">
          ...
          <h3 class="text-base font-semibold tracking-[-0.02em] text-primary">{{ service.title }}</h3>
          <p class="text-[13px] leading-[1.6] text-body">{{ service.desc }}</p>
          <p class="mt-auto pt-2 font-mono text-xs text-accent">{{ service.price }}</p>
        </li>
      </ul>
    </section>
    ...
    <LazyProfessionalPortfolioAndReviews hydrate-on-visible :projects="professional.projects" :reviews="professional.reviewsList" />
  </div>
</template>

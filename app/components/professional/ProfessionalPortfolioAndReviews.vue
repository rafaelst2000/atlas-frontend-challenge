  ...
  <div class="flex flex-col gap-5">
        ...
        <li v-for="project in projects" :key="project.name" class="card overflow-hidden">
          ...
          <div class="flex flex-col gap-2.25 p-4.5">
            ...
            <ul class="flex flex-wrap gap-1.5">
              <li v-for="tech in project.techs" :key="tech" class="rounded-full border border-subtle px-2.25 py-1 font-mono text-[10.5px] text-body">{{ tech }}</li>
            </ul>
          </div>
        </li>
      </ul>
    ...
    <section aria-labelledby="avaliacoes">
      ...
      <ul class="flex flex-col gap-3.5">
        <li v-for="review in reviews" :key="review.author" class="flex flex-col gap-3 rounded-card bg-card p-5.5 shadow-card">
          ...
          <p class="text-[15px] leading-[1.7] text-primary [text-wrap:pretty]">“{{ review.text }}”</p>
          <div class="flex items-center gap-2.75 border-t border-subtle pt-3">
            <span class="flex size-9 items-center justify-center rounded-button border border-medium bg-surface font-mono text-xs text-body" aria-hidden="true">{{ review.initials }}</span>
            <div>
              <p class="text-[13.5px] font-semibold text-primary">{{ review.author }}</p>
              <p class="mt-0.5 text-xs text-tertiary">{{ review.role }}</p>
            </div>
          </div>
        </li>
      </ul>

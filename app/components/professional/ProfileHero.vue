...
<template>
  <section class="relative overflow-hidden rounded-card bg-card p-6 shadow-card">
    ...
    <div class="relative flex flex-wrap items-start gap-5">
      <ProfessionalAvatar :src="professional.photo" :name="professional.name" :initials="professional.initials" :size="96" eager />
      <div class="min-w-0 flex-[1_1_260px]">
        ...
        <h1 class="text-[clamp(28px,6vw,44px)] font-extrabold leading-tight tracking-tight text-primary">{{ professional.name }}</h1>
        <p class="mt-2 text-[clamp(14px,3.6vw,17px)] text-body">{{ professional.role }}</p>

        <div class="mt-4 flex flex-wrap gap-x-5.5 gap-y-3.5 text-[13.5px] text-body">
          <div class="flex items-center gap-1.75">
            <span class="flex gap-0.5" aria-hidden="true">
              <svg v-for="(isFilled, index) in stars" :key="index" width="14" height="14" viewBox="0 0 24 24" :fill="isFilled ? 'var(--color-accent)' : 'rgba(212,160,60,0.35)'"><polygon points="12,2.5 14.9,9 22,9.8 16.7,14.5 18.2,21.5 12,17.9 5.8,21.5 7.3,14.5 2,9.8 9.1,9" /></svg>
            </span>

...
<template>
  ...
  <section class="mx-auto max-w-300 px-5 pb-10 pt-5">
    ...
    <div v-else-if="hasError && !professionals.length" class="rounded-card bg-card px-6 py-12 text-center shadow-card">
      <div class="mx-auto mb-5 flex size-13 items-center justify-center rounded-xl border border-error/40 bg-error/10">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--color-error)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 9v4" /><path d="M12 17h.01" /><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z" /></svg>
      </div>
      <h3 class="text-xl font-bold tracking-[-0.02em] text-primary">Não foi possível carregar os profissionais</h3>
      ...
      <button type="button" class="btn btn-primary mt-6" @click="$emit('retry')">Tentar novamente</button>
    </div>
    ...
    <div v-else-if="professionals.length" :class="{ 'opacity-60 transition-opacity': isLoading }">
      ...
      <div class="mt-8 flex flex-col items-center gap-4">
        ...
        <p v-if="hasLoadMoreError" class="text-sm text-error">Não foi possível carregar mais profissionais. Tente novamente.</p>
